...
import br.edu.pcv.main.Main;

public class CriarBD {

	public static void main(String[] args) throws SQLException {
		Connection connection = DriverManager.getConnection(Main.getUrl());
		if(connection != null) {
	...
	static void criarTabelaCidades(Connection connection) throws SQLException {
		connection.createStatement().execute("create table cidades(nome text not null, latitude text not null, longitude text not null)");
